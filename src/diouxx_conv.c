...
/*   Updated: 2019/02/13 15:42:42 by bwan-nan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_handle_x(t_conv *conv)
{
	...
	if (conv->size.hh)
		str = ft_strdup(ft_llitoa_base((char)va_arg(conv->ap, long long), 16, 1));
	else if (conv->size.h)
		str = ft_strdup(ft_llitoa_base((short)va_arg(conv->ap, long long), 16, 1));
	else if (conv->size.ll)
		str = ft_strdup(ft_llitoa_base(va_arg(conv->ap, long long), 16, 1));
	else if (conv->size.l)
		str = ft_strdup(ft_llitoa_base((long)va_arg(conv->ap, long long), 16, 1));
	else
		str = ft_strdup(ft_llitoa_base(va_arg(conv->ap, int), 16, 1));
	ft_handle_di_p(conv, str);
}
...
void	ft_handle_X(t_conv *conv)
{
	...
	if (conv->size.hh)
		...
		str = ft_strdup(ft_llitoa_base(va_arg(conv->ap, int), 16, 0));
	ft_handle_di_p(conv, str);
}
